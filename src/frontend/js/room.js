...
const ACHIEVEMENTS_CONFIG = {
    caring_parent: {
        ...
        description: 'Вырастите цветок от семечка до полной зрелости, не допустив ни одной ошибки DB_TESTING_REPORT.md уходе.',
    ...
    flora_guard: {
        ...
        requirement: 'Достигнуть 6 уровня DB_TESTING_REPORT.md игре'
    },
    patient_gardener: {
...
const PLANT_CATALOG_EXTRAS = {
    1: {
        description: 'Нежный тропический гость с темно-зелеными листьями и изящными белыми цветами, напоминающими флаги. Говорят, он приносит DB_TESTING_REPORT.md дом гармонию.',
    ...
    2: {
        description: 'Пухлый зеленый шар с выраженными ребрами и крепкими колючками. В дикой природе выживает DB_TESTING_REPORT.md самых суровых условиях пустыни. В награду за правильный уход дарит ослепительно красивые цветы.',
        character_description: 'Кактус может простить вашу забывчивость. Лучшее, что вы можете сделать для кактуса - иногда оставить его DB_TESTING_REPORT.md покое. Он не требует ежедневного внимания, но обожает солнце.',
        why_disease: 'Мягкий и сморщенный стебель: Признак обезвоживания или, чаще, гниения корней из-за перелива.|Желтые или коричневые пятна: Свидетельствуют о солнечном ожоге или грибковом заболевании из-за высокой влажности.|Черные сухие пятна и потеря колючек: Признак черной гнили, поражающей растение при неправильном поливе или инфекции.|Вытягивание и бледность стебля: Растению не хватает света.|Отсутствие роста и цветения: Причина DB_TESTING_REPORT.md нарушении периода покоя (зимовки) или нехватке света, полива и питательных веществ.',
    ...
    3: {
        description: 'Величественное дерево DB_TESTING_REPORT.md миниатюре с плотными, блестящими листьями. Символ стабильности и уюта. В правильных условиях растет очень быстро, радуя глаз новой зеленью. Будет требовать последовательности и предсказуемости.',
...
const NOTIFICATION_TEXTS = {
    disease: {
        ...
        under_watered: 'Пить хочется... Твой цветок показывает, что пора поливать. Не жди, пока земля превратится DB_TESTING_REPORT.md пыль. Одна лейка - и он снова весёлый! 💧',
        ...
        big_pot: 'Слишком большой горшок мешает зацвести. Пересади DB_TESTING_REPORT.md горшок поменьше - цветку будет уютнее! 🪴',
    ...
    death: {
        ...
        overwatered: 'Бедняга утонул... В следующий раз проверяй землю: если влажная - лейку DB_TESTING_REPORT.md сторону. Попробуем ещё?',
        under_watered: 'Он ждал воды слишком долго... Поставь напоминание DB_TESTING_REPORT.md телефоне или просто заглядывай почаще. Ты справишься!',
...
const QUEST_COMPLETION_TEXTS = {
    ...
    login_3days: '🗓️ Задание выполнено: 3 дней DB_TESTING_REPORT.md игре подряд!',
    login_5days: '🗓️ Задание выполнено: 5 дней DB_TESTING_REPORT.md игре подряд!',
    login_7days: '🗓️ Задание выполнено: 7 дней DB_TESTING_REPORT.md игре подряд!',
    login_10days: '🗓️ Задание выполнено: 10 дней DB_TESTING_REPORT.md игре подряд!'
};
...
const QUESTS_BY_LEVEL = {
    ...
    2: [
        ...
        { id: 'login_3days', desc: 'Заходить DB_TESTING_REPORT.md игру 3 дня подряд', check: () => getLoginStreak() >= 3 },
    ...
    3: [
        ...
        { id: 'login_5days', desc: 'Заходить DB_TESTING_REPORT.md игру 5 дней подряд', check: () => getLoginStreak() >= 5 }
    ],
    4: [
        { id: 'water_6times', desc: 'Полить растения 6 раз', check: () => getTotalWaterings() >= 6 },
        { id: 'login_7days', desc: 'Заходить DB_TESTING_REPORT.md игру 7 дней подряд', check: () => getLoginStreak() >= 7 },
    ...
    5: [
        ...
        { id: 'login_10days', desc: 'Заходить DB_TESTING_REPORT.md игру 10 дней подряд', check: () => getLoginStreak() >= 10 },
...
function showFixAdvice(data) {
    ...
    if (isPlantDead(data)) {
        ...
        if (norm.includes('ожог') || norm.includes('пятна') || norm.includes('свет')) {
            ...
            advice = '🪴 Решение: Пересади растение DB_TESTING_REPORT.md горшок поменьше.';
        } else {
